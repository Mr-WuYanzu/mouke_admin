...
<form class="layui-form" method="post"> <!-- 提示：如果你不想用form，你可以换成div等任何一个普通元素 -->
	<input type="hidden" id="chapter_id" name="chapter_id" value="{{$chapterInfo['chapter_id']}}">
  <div class="layui-form-item">
    <label class="layui-form-label">章节名称</label>
    <div class="layui-input-inline">
      <input type="text" id="chapter_name" name="chapter_name" value="{{$chapterInfo['chapter_name']}}" placeholder="请输入章节名称" autocomplete="off" class="layui-input">
    </div>
  </div>
  <div class="layui-form-item">
    ...
    <div class="layui-input-inline">
      <select name="curr_id" id="curr_id" lay-filter="aihao">
        ...
        <option @if($chapterInfo['curr_id']==$v['curr_id']) selected @endif value="{{$v['curr_id']}}">{{$v['curr_name']}}</option>
        @endforeach
      </select>
    </div>
  </div>
  <div class="layui-form-item layui-form-text">
    ...
    <div class="layui-input-block">
      <textarea name="chapter_desc" id="chapter_desc" placeholder="请输入课程介绍" class="layui-textarea">{{$chapterInfo['chapter_desc']}}</textarea>
    </div>
  </div>
  <div class="layui-form-item">
    <div class="layui-input-block">
			...
			$('#sub').click(function(){
				...
				if(obj.chapter_name==''){
					...
					$.ajax({
						...
						method:'post',
						async:false,
						data:{chapter_id:obj.chapter_id,chapter_name:obj.chapter_name},
